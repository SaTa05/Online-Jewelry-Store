...
$sql = "SELECT * FROM jewellery ORDER BY id ASC";
...
$i = $start;
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC) and $i < $end) {
        if ($i == $total_results) {
                break;
        }
        echo "<tr>";
        ...
        echo '<td><a href="deleteprod.php?paginated=yes&id=' . $row['id'] . '">Delete</a></td>';
        echo "</tr>";
        $i++;
}
